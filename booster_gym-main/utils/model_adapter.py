...
class ModelAdapter:
    ...
    def adapt_critic_weights(old_checkpoint: Dict, 
        ...
        if 'critic' in old_checkpoint:
            ...
            for key in first_layer_keys:
                if critic_state[key].shape[1] == old_privileged_obs_dim:
                    # 这是输入层，需要适配
                    old_weight = critic_state[key]  # shape: [hidden_dim, old_privileged_obs_dim]
                    
                    # 创建新的权重矩阵
                    new_weight = torch.zeros(old_weight.shape[0], new_privileged_obs_dim, device=device)
                    
                    # 复制旧的权重到前14维
                    new_weight[:, :old_privileged_obs_dim] = old_weight
                    
                    # 对新增的6维使用小的随机初始化
                    new_weight[:, old_privileged_obs_dim:] = torch.randn(
                        old_weight.shape[0], new_privileged_obs_dim - old_privileged_obs_dim, device=device
                    ) * 0.01
...
def load_and_adapt_checkpoint(checkpoint_path: str,
    ...
    adapted_checkpoint = ModelAdapter.adapt_critic_weights(
        adapted_checkpoint, old_privileged_obs_dim, new_privileged_obs_dim, device
    )
    
    print(f"成功适配检查点：")
    print(f"  Actor: {old_obs_dim}维 -> {new_obs_dim}维")
    print(f"  Critic: {old_privileged_obs_dim}维 -> {new_privileged_obs_dim}维")
